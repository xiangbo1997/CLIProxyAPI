...
// Test case 1: String system prompt is moved to first user message (non-strict mode)
func TestCheckSystemInstructionsWithMode_StringSystemPreserved(t *testing.T) {
	...
	blocks := system.Array()
	// billing + agent + static = 3 blocks; user system is moved to first user message.
	if len(blocks) != 3 {
	...
	// User system message is prepended to first user message in non-strict mode.
	firstMsg := gjson.GetBytes(out, "messages.0.content").String()
	if !strings.Contains(firstMsg, "You are a helpful assistant.") {
		t.Fatalf("user system not found in first user message, got %q", firstMsg)
	}
}

// Test case 2: Strict mode keeps only injected blocks (3 total)
func TestCheckSystemInstructionsWithMode_StringSystemStrict(t *testing.T) {
	...
	// billing + agent + static = 3 blocks; user system is dropped in strict mode.
	if len(blocks) != 3 {
...
func TestCheckSystemInstructionsWithMode_EmptyStringSystemIgnored(t *testing.T) {
	...
	blocks := gjson.GetBytes(out, "system").Array()
	// billing + agent + static = 3 blocks; empty system is ignored.
	if len(blocks) != 3 {
		t.Fatalf("empty string system should produce 3 blocks, got %d", len(blocks))
	}
}

// Test case 4: Array system prompt - user system is moved to first user message
func TestCheckSystemInstructionsWithMode_ArraySystemStillWorks(t *testing.T) {
	...
	firstMsg := gjson.GetBytes(out, "messages.0.content").String()
	if !strings.Contains(firstMsg, "Be concise.") {
		t.Fatalf("user system prompt not found in first user message, got %q", firstMsg)
	}
}
...
func TestCheckSystemInstructionsWithMode_StringWithSpecialChars(t *testing.T) {
	...
	// In non-strict mode the user system is prepended to the first user message, not kept as block[2].
	firstMsg := gjson.GetBytes(out, "messages.0.content").String()
	if !strings.Contains(firstMsg, `Use <xml> tags`) {
		t.Fatalf("user system not found in first message, got %q", firstMsg)
	}
}
